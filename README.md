...
Options:
      ...
      --skip         Skip building files whose names contain the given filter.
...
The `--skip` flag can be used to skip specific files during compilation.

For example, to skip all files with the `.t.sol` extension:

```
zkforge zk-build --skip test
```

To skip all files with the `.s.sol` extension:

```
zkforge zk-build --skip script
```

To skip all files with the `.t.sol` and `.s.sol` extensions:

```
zkforge zk-build --skip test script
```

to skip all files with the `.t.sol` and `.s.sol` extensions and the `SimpleFactory.sol` file:

```
zkforge zk-build --skip test script SimpleFactory
```

> `.sol` extension is optional when naming files to skip
